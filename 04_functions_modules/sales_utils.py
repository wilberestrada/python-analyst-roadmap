...
def load_sales_text(path: Path) -> Iterable[Tuple[str, int]]:
    ...
    with path.open("r", encoding="utf-8") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            parts = line.split(",")
            if len(parts) != 2:
                print(f'[WARN] Line {lineno}: wrong format -> {line!r}')
                continue
            product, qty_str = parts[0].strip(), parts[1].strip()
            if not product:
                print(f"[WARN] Line {lineno}: empty product -> {line!r}")
                continue
            try:
                qty = int(qty_str)
            except ValueError:
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        for lineno, row in enumerate(reader, start=1):
            ...
            try:
                ...
                print(f"[WARN] Line {lineno} quantity is not integer -> {qtry_str}")
                continue
            yield product, qty
...
def save_summary_text(path: Path, summary: Dict[str, int], order: str = "alpha") -> None:
    ...
    """
    if order =="desc":
        items = sorted(summary.items(), key=lambda kv[1], reverse=True)
    else:
